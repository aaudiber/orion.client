	...
	EditorCommandFactory.prototype = {
		...
		_handleBindingChanges: function(textView, args) {
			...
			if (actions.indexOf(args.id) !== -1) {
				textView.overrideKeyBinding(args.id, args.newBinding, args.prevBinding);
			}
		},
					...
					if (bindings.length > 0) {
						for (var j = 0; j < bindings.length; j++) {
							...
							if (binding.scopeName) {
								...
								keyAssist.createItem(bindingString, actionDescription.name, actionID, execute(actionID), binding);
							}
						}
					} else {
						keyAssist.createItem("---", actionDescription.name, actionID, execute(actionID)); //$NON-NLS-1$
					}
				}
				for (var scopedBinding in scopes) {
					if (scopes[scopedBinding].length) {
						...
						for (var k = 0; k < scopes[scopedBinding].length; k++) {
							...
							keyAssist.createItem(binding.bindingString, binding.name, binding.name, binding.execute);
						}
					}
				}
			}
